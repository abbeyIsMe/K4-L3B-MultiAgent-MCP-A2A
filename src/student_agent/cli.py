...
async def _show_tools(root: Path, metadata: bool = False) -> None:
    settings = Settings.load(root)
    contracts = Contracts(root / "contracts" / "schemas")
    async with connect_gateway(settings.mcp_endpoint, settings.team_api_key, contracts) as gateway:
        if metadata:
            print(json.dumps(await gateway.list_tool_definitions(), ensure_ascii=False, indent=2))
        else:
...
def parser() -> argparse.ArgumentParser:
    ...
    tools.add_argument(
        "--metadata", action="store_true", help="include descriptions and input schemas"
    )
...
def main() -> None:
    ...
    try:
        if args.command == "validate-inputs":
            ...
            asyncio.run(_show_tools(root, args.metadata))
